...
if [[ ${PSQL_MODE} == standalone || ${PSQL_MODE} == master ]]; then
  ...
  if [[ -n ${DB_NAME} ]]; then
    for db in $(awk -F',' '{for (i = 1 ; i <= NF ; i++) print $i}' <<< "${DB_NAME}"); do
      ...
      # beginning of adminpack...
      if [[ -n ${DB_ADMINPACK} ]]; then
        echo "Creating AdminPack \"${db}\" for user \"${DB_USER}\"..."
        echo "CREATE EXTENSION if not exists adminpack;" |
          sudo -Hu ${PG_USER} ${PG_BINDIR}/postgres --single \
            -D ${PG_DATADIR} -c config_file=${PG_CONFDIR}/postgresql.conf >/dev/null
      fi
      # end of adminpack
...
if [[ -n ${DB_NAME} ]]; then
  sudo -Hu ${PG_USER} /usr/lib/postgresql/${PG_VERSION}/bin/pg_ctl start -w -D ${PG_DATADIR}
  sleep 5
  for db in $(awk -F',' '{for (i = 1 ; i <= NF ; i++) print $i}' <<< "${DB_NAME}"); do
    echo "Creating PostGIS extension for \"${db}\"...";
    sudo -Hu ${PG_USER} psql -d ${db} -c "create extension if not exists postgis;"
    sudo -Hu ${PG_USER} psql -d ${db} -c "create extension if not exists cube;"
    sudo -Hu ${PG_USER} psql -d ${db} -c "create extension if not exists earthdistance;"
    echo "OH extension got created for \"${db}\"";
  done

  sudo -Hu ${PG_USER} /usr/lib/postgresql/${PG_VERSION}/bin/pg_ctl stop -w -D ${PG_DATADIR}
fi

echo "Complete with setup!"
